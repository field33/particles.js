...
var pJS = function (tag_id, params) {
  ...
  this.pJS = {
    ...
    particles: {
      ...
      // Percentage of nodes that will have a text attached
      text_percentage: 100,
      shape: {
        type: "circle",
        stroke: {
  ...
  pJS.fn.particle = function (color, opacity, text, position) {
    /* size */
    this.radius =
    ...
    if (text) {
      var will_get_text = Math.random() < pJS.particles.text_percentage / 100;
      if (will_get_text) {
        var text_selected =
          text[Math.floor(Math.random() * pJS.particles.text.length)];
  ...
  pJS.fn.particle.prototype.draw = function () {
    ...
    if (p.text) {
      ...
      pJS.canvas.ctx.fillStyle = "#2e2d2c";
